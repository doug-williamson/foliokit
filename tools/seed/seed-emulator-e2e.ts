/**
 * Playwright / e2e baseline — Auth user plus full tenant Firestore fixtures.
 * Not run by `npm run dev:all`. Use `npm run seed:e2e` or e2e globalSetup.
 */
...
async function seed(): Promise<void> {
  try {
    console.log('[seed:emulator-e2e] Creating admin user dev.foliokit@gmail.com...');
    ...
    if (errors.length) {
      console.warn('[seed:emulator-e2e] Admin user import warnings:', errors.map((e) => e.error.message));
    }

    console.log(`[seed:emulator-e2e] Writing /tenants/${TENANT_ID}...`);
    ...
    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/authors/author-1...`);
    ...
    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/tags/tag-web...`);
    await tenant('tags').doc('tag-web').set(
      { id: 'tag-web', label: 'Web', slug: 'web' },
      { merge: false }
    );

    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/tags/tag-angular...`);
    await tenant('tags').doc('tag-angular').set(
      { id: 'tag-angular', label: 'Angular', slug: 'angular' },
      { merge: false }
    );

    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/site-config/${TENANT_ID}...`);
        ...
        pages: {
          ...
          about: {
            ...
            seo: {
              ...
              description:
                'Learn more about Dev Author, an engineer and writer focused on Angular and Firebase.',
            },
          },
          links: {
            enabled: true,
    ...
    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/pages/links...`);
    ...
    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/posts/post-1 (published)...`);
    ...
    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/posts/post-2 (published)...`);
    ...
    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/posts/post-3 (scheduled)...`);
    await tenant('posts').doc('post-3').set(
      {
        ...
        excerpt:
          'Tailwind CSS v4 brings a new engine and CSS-first configuration. Here is what to expect.',
        content:
    ...
    console.log(`[seed:emulator-e2e] Writing tenants/${TENANT_ID}/posts/post-4 (draft)...`);
    ...
    console.log(
      `[seed:emulator-e2e] Done. Tenant: ${TENANT_ID}. Auth + Firestore baseline for e2e.`
    );
  } catch (err) {
    console.error('[seed:emulator-e2e] Error:', err);
    process.exit(1);
  }
}
