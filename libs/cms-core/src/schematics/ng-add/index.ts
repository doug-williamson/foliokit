...
function getSourceRoot(tree: Tree, project: string | undefined): string {
...
function getProjectName(tree: Tree, project: string | undefined): string {
  if (!tree.exists('angular.json')) {
    return 'my-app';
  }
  const workspace = JSON.parse(tree.read('angular.json')!.toString('utf-8'));
...
function firebaseJson(projectName: string): string {
...
function firebaseRc(firebaseProject: string): string {
...
export function ngAdd(options: NgAddOptions): Rule {
  return (tree: Tree, context: SchematicContext) => {
    ...
    if (tree.exists(envPath)) {
      ...
      context.logger.warn(
        '⚠  Replace placeholder values in environment.ts with your Firebase project credentials before running the app.',
      );
    }
    ...
    if (tree.exists(firebaseRcPath)) {
      ...
      if (!firebaseProject) {
        context.logger.warn(
          '⚠  .firebaserc was generated with placeholder values. Update with your Firebase project ID.',
        );
      }
    }
